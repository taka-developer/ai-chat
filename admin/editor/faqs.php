...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle') {
        $id = (int)($_POST['id'] ?? 0);
        $pdo->prepare('UPDATE faqs SET is_active = 1 - is_active WHERE id = ? AND client_id = ?')->execute([$id, $clientId]);
    }
    ...
    if ($action === 'edit') {
        ...
        if ($id && $question && $answer) {
            ...
            $pdo->prepare(
                'UPDATE faqs SET category=?, question=?, answer=?, keywords=?, priority=?, updated_at=NOW() WHERE id=? AND client_id=?'
            )->execute([$category, $question, $answer, $keywords, $priority, $id, $clientId]);
...
$faqs = $pdo->prepare(
    'SELECT * FROM faqs WHERE client_id = ? ORDER BY priority DESC, updated_at DESC'
);
$faqs->execute([$clientId]);
$faqs = $faqs->fetchAll();

$categories = $pdo->prepare('SELECT DISTINCT name FROM faq_categories WHERE client_id = ? ORDER BY name');
$categories->execute([$clientId]);
...
<div class="card">
  ...
  <form method="post">
    ...
    <div class="form-group"><label>キーワード（空欄なら自動生成）</label><input type="text" name="keywords" placeholder="例: 営業時間,定休日"></div>
...
<div class="card">
  <table>
    <thead><tr><th>カテゴリ</th><th>質問</th><th>状態</th><th>優先度</th><th>更新日</th><th></th></tr></thead>
    <tbody>
    <?php foreach ($faqs as $f): ?>
      <tr>
        <td><?= htmlspecialchars($f['category'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
        <td><?= htmlspecialchars(mb_substr($f['question'], 0, 60), ENT_QUOTES, 'UTF-8') ?><?= mb_strlen($f['question']) > 60 ? '…' : '' ?></td>
        <td>
        ...
        <td><?= htmlspecialchars(substr($f['updated_at'], 0, 10), ENT_QUOTES, 'UTF-8') ?></td>
            ...
            <form method="post">
              ...
              <div style="display:grid;grid-template-columns:1fr 120px;gap:12px">
                <div class="form-group" style="margin-bottom:0">
                  <label>カテゴリ</label>
                  <input type="text" name="category" value="<?= htmlspecialchars($f['category'] ?? '', ENT_QUOTES, 'UTF-8') ?>" list="cat-list">
                </div>
                <div class="form-group" style="margin-bottom:0">
                  <label>優先度</label>
...
<?php
adminLayout('FAQ管理', ob_get_clean(), 'editor');
